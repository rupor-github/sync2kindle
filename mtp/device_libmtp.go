...
// void restore_stdio(s2k_stdio_backup backup) {
//     fflush(stdout);
//     fflush(stderr);
...
func pickDevice(serial string, _ bool, log *zap.Logger) (usbIDs *common.PnPDeviceID, dev *C.LIBMTP_mtpdevice_t, err error) {
	defer func() {
		if err != nil && dev != nil {
	...
	// libmtp writes detection chatter directly to stderr; keep terminal output under logger control.
	save := C.null_stdio()
	errno := C.LIBMTP_Detect_Raw_Devices(&rawdevs, &numdevs)
	C.restore_stdio(save)
...
func releaseMTPDevice(dev *C.LIBMTP_mtpdevice_t) {
	...
	if runtime.GOOS == "darwin" {
		// On macOS, explicit libmtp release can leave some Kindles unusable until re-plugged.
		return
	}
